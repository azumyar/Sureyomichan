...
### レス文章に付ける特殊タグ  
引用符から始まるレス行（>から始まる行）に追加の特殊タグをつけることが出来ます。
例えば設定に「#」と入れた場合、「>テキスト… 」が「#>テキスト… 」となります。
デフォルトは空欄です。  
...
### そうだね通知
スレ文(0レス目)にそうだねが入ると通知します。  
この機能はそうだねが入るとスレ寿命が延びる仕様に対応するためimgでは通知しません。
...
設定方法について  
※管理者権限が必要です。  
「URLスキーマ登録」をクリックすると管理者への昇格ダイアログの「このアプリがデバイスに変更を加えることを許可しますか」  
 と画面に表示されるので「はい」をクリックします。

![-](/images/usage/setting_055_schema_set01.png)

登録が成功するとスレ詠みちゃんの画面下部に「URLスキーマを登録しました」の文字が表示されます。  
...
こちらも登録と同様、管理者権限が必要で削除時に管理者への昇格ダイアログが表示されます。  
削除すると、スレ詠みちゃん画面下部に「URLスキーマを解除しました」の文字が表示されます。  
